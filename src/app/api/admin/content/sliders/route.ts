...
import { slidersArraySchema } from "@/lib/validations";
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // Validate input
        const validation = slidersArraySchema.safeParse(items);
        if (!validation.success) {
            return badRequestResponse(validation.error.flatten().fieldErrors);
        }

        await adminDb.collection("content").doc("sliders").set({ items: validation.data });
        return NextResponse.json({ success: true });
    } catch (error) {
        console.error("Failed to save sliders:", error);
        return NextResponse.json({ error: "Failed to save sliders" }, { status: 500 });
    }
}
