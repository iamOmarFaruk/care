...
import { footerSchema } from "@/lib/validations";
...
export async function GET(request: NextRequest) {
    ...
    try {
        const doc = await adminDb.collection("content").doc("footer").get();
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // Validate input
        const validation = footerSchema.safeParse(data);
        if (!validation.success) {
            return badRequestResponse(validation.error.flatten().fieldErrors);
        }

        await adminDb.collection("content").doc("footer").set(validation.data);
        return NextResponse.json({ success: true });
    } catch (error) {
        console.error("Failed to save footer content:", error);
        return NextResponse.json({ error: "Failed to save footer content" }, { status: 500 });
    }
}
